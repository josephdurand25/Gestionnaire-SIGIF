...
export const DossierCandidatureProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const fetchDossiers = useCallback(
    async (page = 1, limit = 10) => {
      ...
      try {
        const response = await api.get(`/api/dossiers-candidature?page=${page}&limit=${limit}`, {
  ...
  const fetchDossierById = useCallback(
    async (id: number) => {
      dispatch({ type: "SET_PROCESSING", payload: true });
      try {
        const response = await api.get(`/api/dossiers-candidature/${id}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        });
        if (response.data?.success) {
  ...
  const createDossier = useCallback(
    async (data: Partial<IDossierCandidature>) => {
      ...
      try {
        const response = await api.post("/api/dossiers-candidature", data, {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        });
  ...
  const updateDossier = useCallback(
    async (id: number, data: Partial<IDossierCandidature>) => {
      ...
      dispatch({ type: "RESET_ERRORS" });
      try {
        const response = await api.put(`/api/dossiers-candidature/${id}`, data, {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        });
  ...
  const deleteDossier = useCallback(
    async (id: number) => {
      dispatch({ type: "SET_PROCESSING", payload: true });
      try {
        const response = await api.delete(`/api/dossiers-candidature/${id}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        });
  ...
  const fetchDossierStats = useCallback(
    async () => {
      try {
        const response = await api.get("/api/dossiers-candidature/statistics", {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        });
